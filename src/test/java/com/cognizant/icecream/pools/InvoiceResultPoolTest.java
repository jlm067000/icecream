...
public class InvoiceResultPoolTest extends PoolTest<MutableServiceResult<Invoice>> {

    private static final String MESSAGE = "";
    private static final Invoice PAYLOAD = new Invoice();

    private InvoiceResultPool pool;

    @Before
    public void setup() {

        super.setup(ResultFactory.createMutableServiceResult());
        pool = new InvoiceResultPool(factory, config, abandonedConfig);
    }

    @Test(expected = PoolCapacityException.class)
    public void testCreateBeyondCapacity() throws Exception {

        MutableServiceResult<Invoice> result = pool.createResult(true, MESSAGE, PAYLOAD);
